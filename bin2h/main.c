...
static void print_help(const char *name) {
  ...
  fprintf(stdout,
          "USAGE: %s [-i <input>] [-o <output>] [-n <name>] [-d] [-e] [-s] "
          "[-z]\n\n",
          name);
  fprintf(
      stdout,
      ...
      "	-e				Output a variable that holds a pointer "
      "to the last element of the data\n"
...
static void generate_output(FILE *input_s, FILE *output_s, const char *name,
                            int zero_terminate, int decimal, int end_variable,
                            int size_variable) {
  int byte;
  unsigned int i = 0U;
...
int main(int argc, char *argv[]) {
  ...
  generate_output(input_f ? input_f : stdin, output_f ? output_f : stdout, name,
